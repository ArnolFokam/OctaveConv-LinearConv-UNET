...
from models.linearconv.blocks.conv_block import LinearConvBlock, LinearTransposeConvBlock
from models.octave.utils.spatial_merging import padding_into_max_shape
from models.octave.utils.spatial_merging import cropping_into_min_shape


class OctaveLinearConv2d(nn.Module):
    """Octave convolution."""
    ...
    def __init__(self, in_channels, out_channels, in_alpha, out_alpha,
                 ...
                 downsample_mode='avg', upsample_mode='bilinear', **kwargs):
        super(OctaveLinearConv2d, self).__init__()
        assert 0 <= in_alpha <= 1
        ...
        if self.low_only_input or self.low_only_output:
            ...
            self.conv_h2h = LinearConvBlock(in_channels_h,
                                            out_channels_h,
        ...
        if self.high_only_input or self.high_only_output:
            ...
            self.conv_l2l = LinearConvBlock(in_channels_l,
                                            out_channels_l,
                                            kernel_size=kernel_size,
                                            stride=stride,
                                            padding=padding,
        ...
        if self.low_only_input or self.high_only_output or self.depth_wise:
            ...
            if downsample_mode == 'avg':
                ...
                self.downsample_h2l = LinearConvBlock(in_channels=in_channels_h,
            ...
            self.conv_h2l = LinearConvBlock(in_channels_h,
                                            out_channels_l,
                                            kernel_size=kernel_size,
                                            stride=stride,
                                            padding=padding,
        ...
        if self.high_only_input or self.low_only_output or self.depth_wise:
            ...
            self.conv_l2h = LinearConvBlock(in_channels_l,
                                            out_channels_h,
                                            kernel_size=kernel_size,
                                            stride=stride,
                                            padding=padding,
            ...
            if upsample_mode in ['nearest', 'bilinear']:
                ...
                self.upsample_l2h = LinearTransposeConvBlock(
...
class OctaveLinearConvTranspose2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, in_alpha, out_alpha,
        ...
        super(OctaveLinearConvTranspose2d, self).__init__()
        assert 0 <= in_alpha <= 1
        self.low_only_input = bool(in_alpha == 1)
        self.high_only_input = bool(in_alpha == 0)
        self.multi_res_input = bool(0 < in_alpha < 1)
        ...
        if self.low_only_input or self.low_only_output:
            ...
            self.conv_transposed_h2h = LinearTransposeConvBlock(in_channels=in_channels_h,
                                                                out_channels=out_channels_h,
                                                                kernel_size=kernel_size,
                                                                stride=stride,
                                                                padding=padding,
        ...
        if self.high_only_input or self.high_only_output:
            ...
            self.conv_transposed_l2l = LinearTransposeConvBlock(in_channels=in_channels_l,
        ...
        if self.low_only_input or self.high_only_output or self.depth_wise:
            ...
            self.conv_transposed_h2l = LinearTransposeConvBlock(in_channels=in_channels_h,
                                                                out_channels=out_channels_l,
                                                                kernel_size=kernel_size,
                                                                stride=stride,
                                                                padding=padding,
        ...
        if self.high_only_input or self.low_only_output or self.depth_wise:
            ...
            self.conv_transposed_l2h = LinearTransposeConvBlock(in_channels=in_channels_l,
                                                                out_channels=out_channels_h,
                                                                kernel_size=kernel_size,
                                                                stride=stride,
                                                                padding=padding,
